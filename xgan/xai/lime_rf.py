...
class LimeRandomForest(LimeModel):
	...
	def explain_sample(self, explained_sample, formatted_idx, batch_generator, prefix_path, explanation_config, generation_config):
		X, y, weights = self._convert_data_to_pandas(explained_sample, batch_generator)
		...
		self.model.fit(X_sampled, y_samples, sample_weight=weights_sampled)
	...
	def _explain_model(self, generation_config, formatted_idx, prefix_path):
		...
		for estimator_idx, plot_idx in enumerate(np.ndindex(shape)):
			...
			plot_tree(estimator, ax=axes[plot_idx])
